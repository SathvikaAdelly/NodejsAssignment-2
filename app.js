...
// all te people who the logged user if following
...
app.get(
  '/tweets/:tweetId/',
  authenticateToken.applyfollows,
  async (request, response) => {
    ...
    const {replies} = await db.get(`
        select count(reply_id) as replies fromm reply where tweet_id = ${tweetId}; `)
    response.send({tweet, likes, replies, dateTime: date_time})
  },
)

// get all likes of a tweet with tweet id oif the user follows the tweeter
app.get(
...
// get all replies of a tweet of a tweet with tweet id if the user follows the tweetr
...
app.get(
  ...
  async (request, response) => {
    const {tweetId} = request.params
    const replies =
      await db.all(`select user.name, reply.reply from reply natural join user 
...
app.get('/user/tweets/', authenticateToken, async (request, response) => {
  const myTweets = await db.all(`
    select 
    tweet.tweet,
    count(district like.like_id) as likes,
    count(district reply.reply_id) as replies,
    tweet.date_time
    from
    tweet
